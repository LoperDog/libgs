...
void app::TCPServer::RecvHandling(const boost::system::error_code& error, const size_t si) {
  std::cout << "Server : doing recv! : " << si << std::endl;;
...
void app::TCPClient::testSend()
{
  ...
  TestData.size = sizeof(TestData);
  std::cout << "before the Send Size : " << TestData.size << std::endl;

  char* buf = new char[sizeof(TestData)];

  std::cout << "Test Data Size : " << sizeof(TestData) << std::endl;
  std::cout << "buf Data Size : " << std::strlen(buf) << std::endl;
